...
class MainActivity : AppCompatActivity() {
    ...
    private fun getUser(){
        FirebaseFirestore.getInstance().collection("Users").document("user:${App.dm.getUserKey()}").get()
            .addOnCompleteListener {
                if (it.isSuccessful){
                    userData.nick = it.result.data?.get("nick").toString()
                    userData.listPlatform = it.result.data?.get("listPlatform").toString().replace("[", "").replace("]", "").replace(",", "").split(" ")
